...
ADMIN_ID = 6416459996
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8941048533:AAFJUlaY7aBxd3J7nZOMBAck9MyI7eYBlW0")
CHANNEL_USERNAME = "@TALIM_ADMINII"

logging.basicConfig(level=logging.INFO)

# Conversation handler bosqichlari
LANG, TEMPLATE, NAME, PHONE, PHOTO, SKILLS, EXPERIENCE, LANGUAGES_EXP, PORTFOLIO, CERTIFICATES = range(10)
...
def generate_pdf(data, photo_bytes=None):
    ...
    theme_color = colors.HexColor("#1A365D") if data.get("template") == "modern" else colors.HexColor("#2B6CB0")
    ...
    # Bo'limlar xatoligi to'g'rilandi
    lang = data.get("lang", "uz")
    sections = [
        ("💡 " + ("Ko'nikmalar" if lang == "uz" else "Навыки"), data.get("skills")),
        ("💼 " + ("Ish tajribasi" if lang == "uz" else "Опыт работы"), data.get("experience")),
        ("🌐 " + ("Tillar" if lang == "uz" else "Языки"), data.get("languages")),
        ("🔗 " + ("Portfolio" if lang == "uz" else "Портфолио"), data.get("portfolio")),
        ("📜 " + ("Sertifikatlar" if lang == "uz" else "Сертификаты"), data.get("certificates")),
    ]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_subscribed:
        ...
        await update.message.reply_text("Botdan foydalanish uchun rasmiy kanalimizga a'zo bo'ling:", reply_markup=InlineKeyboardMarkup(keyboard))
        return ConversationHandler.END

    keyboard = [["🇺🇿 O'zbekcha", "🇷🇺 Русский"]]
    await update.message.reply_text("Tilni tanlang / Выберите язык:", reply_markup=ReplyKeyboardMarkup(keyboard, resize_keyboard=True, one_time_keyboard=True))
    return LANG
...
async def check_sub_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_subscribed:
        ...
        # Foydalanuvchini /start ga yo'naltiramiz
...
async def create_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(TEXTS[lang]["choose_template"], reply_markup=ReplyKeyboardMarkup(keyboard, resize_keyboard=True, one_time_keyboard=True))
    return TEMPLATE
...
async def get_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(TEXTS[lang]["ask_photo"], reply_markup=ReplyKeyboardMarkup(keyboard, resize_keyboard=True, one_time_keyboard=True))
    return PHOTO
...
async def get_certificates(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    pdf_file = generate_pdf(context.user_data, context.user_data.get("photo_bytes"))
...
async def broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for uid in users:
        try:
            ...
            sent += 1
            await asyncio.sleep(0.05) # Telegram bloklamasligi uchun pauza qo'shildi
        except Exception:
            pass
...
def main():
    ...
    conv_handler = ConversationHandler(
        entry_points=[CommandHandler("create", create_start), CommandHandler("start", start)],
        states={
